...
Процедура pmGenerate(pSpreadsheet) Экспорт
	...
	ReportBuilder.Parameters.Вставить("qEmptyContract", Справочники.Договора.EmptyRef());
...
Процедура pmInitializeReportBuilder() Экспорт
	...
	|	ISNULL(БухРеализацияУслуг.ДокОснование.Контрагент, &qEmptyCustomer) AS Контрагент,
	|	ISNULL(БухРеализацияУслуг.ДокОснование.Договор, &qEmptyContract) AS Договор,
	...
	|	AND (БухРеализацияУслуг.ДокОснование.Контрагент IN HIERARCHY (&qCustomer)
	|			OR &qCustomerIsEmpty)
	|	AND (БухРеализацияУслуг.ДокОснование.Договор IN HIERARCHY (&qContract)
	|			OR &qContractIsEmpty)
	...
	|	ISNULL(БухРеализацияУслуг.ДокОснование.Контрагент, &qEmptyCustomer),
	|	ISNULL(БухРеализацияУслуг.ДокОснование.Договор, &qEmptyContract),
	...
	|	ISNULL(ПрогнозРеализации.Recorder.Контрагент, &qEmptyCustomer) AS Контрагент,
	|	ISNULL(ПрогнозРеализации.Recorder.Договор, &qEmptyContract) AS Договор,
	...
	|	ISNULL(ПрогнозРеализации.Recorder.CheckInDate, &qEmptyDate) AS CheckInDate,
	|	ISNULL(ПрогнозРеализации.Recorder.CheckOutDate, &qEmptyDate) AS CheckOutDate,
	|	CASE
	...
	|	AND (ПрогнозРеализации.Recorder.Контрагент IN HIERARCHY (&qCustomer)
	|			OR &qCustomerIsEmpty)
	|	AND (ПрогнозРеализации.Recorder.Договор IN HIERARCHY (&qContract)
	|			OR &qContractIsEmpty)
	...
	|	ISNULL(ПрогнозРеализации.Recorder.Контрагент, &qEmptyCustomer),
	|	ISNULL(ПрогнозРеализации.Recorder.Договор, &qEmptyContract),
	...
	|	ISNULL(ПрогнозРеализации.Recorder.CheckInDate, &qEmptyDate),
	|	ISNULL(ПрогнозРеализации.Recorder.CheckOutDate, &qEmptyDate),
	|	CASE
	|		WHEN NOT ПрогнозРеализации.Recorder.ReservationStatus IS NULL 
	|			THEN ПрогнозРеализации.Recorder.ReservationStatus
	|		ELSE ISNULL(ПрогнозРеализации.Recorder.ResourceReservationStatus, &qEmptyResourceReservationStatus)
	...
	ReportBuilder.HeaderText = NStr("EN='Check Контрагент guests';RU='Сверка начислений по гостям от контрагентов';de='Abgleich der Berechnungen nach Gästen von den Vertragspartnern'");
